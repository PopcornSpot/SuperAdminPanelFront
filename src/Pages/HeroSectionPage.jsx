...
const HeroSection = () => {
    ...
    try {
      await axios
        ...
        .then((res) => {
          ...
          setBanner((prevState) => prevState.filter((value) => value._id !== _id));
        })
        .catch((err) => {
  ...
  return (
    <>
      <SidebarComponent />
      <div className="ml-60 xl:ml-64 max-sm:ml-0 flex-1 p-5 min-h-screen bg-gray-900">
        <div className="text-3xl font-bold h-20 text-gray-100 mb-6 mt-16 sm:mt-0 px-6 flex flex-col sm:flex-row gap-4 justify-between items-center">
            ...
            >
              ...
              <div className="flex gap-2 mt-4">
                <Link to={`/updatebanner/${card._id}`}>
                  <button className="hidden sm:block bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
                    Edit
                  </button>
                </Link>
                <button
